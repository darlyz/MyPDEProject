...
// remove remv_SN from dest[] and length--
void remove_node(int **dest, int *length, int remv_SN) {

    int index = Binary_Search_(*dest, *length, remv_SN);

    if (index == -1)
        return;

    else {
        memmove( (*dest) +index, (*dest) +index + 1, ((*length) - index) * sizeof(int) );
